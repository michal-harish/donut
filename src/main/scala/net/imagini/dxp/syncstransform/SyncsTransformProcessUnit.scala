...
import org.apache.donut.DonutAppTaskTransform
...
class SyncsTransformProcessUnit(config: Configuration, logicalPartition: Int, totalLogicalPartitions: Int, topics: Seq[String])
  extends DonutAppTaskTransform(config, logicalPartition, totalLogicalPartitions, topics) {
  ...
  override def awaitingTermination: Unit = {
    println(s"SyncsTransformProcessUnit datasync(${counterReceived.get}) => filter(${counterFiltered.get}) => ...")
  }
